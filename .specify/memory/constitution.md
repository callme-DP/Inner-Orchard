...
### 10. Leadership 沟通记录（宪章级）
- 目的：当出现反复往返或歧义时，必须在 `specs/Leadership.md` 留痕，记录争议点与共识，减少噪音。
- 记录格式：二级标题 = 日期 + 主题；正文包含问题摘要、双方观点、最新共识/决定、待办提醒。
- 范围：仅限沟通对齐/决策，不写功能实现细节或产物。

### 11. DRY原则（Don't Repeat Yourself）
...
### 20. 协作日志约定
- 对话/操作完成后，需在对应 feature 的 `log.md` 简要记录关键改动（目录调整、脚本新增、产物路径），保持追溯性；若当日已有同主题日志可追加同一日期节。

### 21. 系统治理约定引用
- 文档角色/权限、升级路径、必问人类条件见 `specs/system-governance.md`，按其中规则执行；未经授权不得修改本宪章。

### 22. 提交分组提示
- 当暂存文件超过 15 个时，先提示是否按功能分批 commit；允许先本地分组 commit，未完成的可暂不 push，避免混杂。

### 22. 文档段落精简
- 当某一层级下的列表（如 AGENTS.md 二级标题下的 “-” 项）超过 5 个，应抽取子标题或分组，避免过长的平铺列表。

### 23. 子目录协作检查
- 操作带子目录的功能时，若父级 `spec.md` 未索引或缺少协作指导，必须提示补充/完善后再继续执行子目录操作。
